...
def layout_budget(viewport_h: int) -> dict[str, int]:
    ...
    # Per-row ratios. Decision row trimmed slightly so verdict + KPI strip
    # stays compact; pattern row gets most space for its small-multiples grid.
    decision_card = max(240, int(available * 0.26))   # 3 cards same height
    pattern_card  = max(310, int(available * 0.42))   # 2 cards same height
    drivers_card  = max(220, int(available * 0.28))   # 4 cards same height
